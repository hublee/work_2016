...
import com.zeustel.top9.widgets.RippleBackground;

import java.util.ArrayList;
import java.util.List;

public class TempScreenshotExhibition extends WrapNotAndHandleActivity implements OnClickListener, SensorListener.OnSensorListener {
    ...
    public int anim_Type = 4;// 1、箭头；2、方形；3、闪烁;4、temp
            ...
            switch (msg.what) {
                ...
                case MSG_GROUP_UPDATE:
                    if (msg.obj != null) {
                        ...
                        controlGroup.addView(view);
                        if (4 == anim_Type) {
                            if (view instanceof RippleBackground) {
                                ((RippleBackground) view).startRippleAnimation();
                            }
                        } else if (mScreenShotControl.getW() == mScreenShotControl.getH() || 3 == anim_Type) {
                            // 闪烁
    ...
    double zoomWidth = 0;
    double zoomHeight = 0;
    int w, h, left, top, min, halfMin = 0;
    ...
    private void fillInGroup() {
        Tools.pool.submit(new Runnable() {
            ...
            public void run() {
                ...
                // TODO Auto-generated method stub
                zoomWidth = ((double) (displayMetrics[0] * 100 / mGameImages.getWidth())) / 100;
                zoomHeight = ((double) (displayMetrics[1] * 100 / mGameImages.getHeight())) / 100;

                zoom = (zoomWidth + zoomHeight) / 2;
                if (mControlList != null && !mControlList.isEmpty()) {
                    for (final GameImageButtons mControlItem : mControlList) {
                        View itemView = inflater.inflate(R.layout.screenshot_item_btn, null);
                        //                        Button mBtn = new Button(getApplicationContext());
                        //                        mBtn.setBackgroundColor(Color.TRANSPARENT);

                        w = (int) (mControlItem.getW() * zoom);
                        h = (int) (mControlItem.getH() * zoom);
                        left = (int) (mControlItem.getLeft() * zoom);
                        top = (int) (mControlItem.getTop() * zoom);
                        ...
                        if (4 == anim_Type) {
                            ...
                            left += w / 2 - halfMin;
                            top += h / 2 - halfMin;
                            params = new RelativeLayout.LayoutParams(min, min);
                            params.leftMargin = left;
                            params.topMargin = top;

                        } else if (w == h || 3 == anim_Type) {
                        ...
                        itemView.setTag(mControlItem.getNextImageId());
                        itemView.setOnClickListener(TempScreenshotExhibition.this);
                        itemView.setLayoutParams(params);
                        ScreenShotControl mScreenShotControl = new ScreenShotControl(left, top, w, h, itemView);
