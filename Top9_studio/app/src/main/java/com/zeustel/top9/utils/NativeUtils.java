...
import com.zeustel.top9.result.TestSubList;
...
public class NativeUtils {
    ...
    public static void importChatFaces(final Context context) {
        ...
        try {
            ...
            for (int i = 0; i < names.length; i++) {
                name = names[i];
                open = context.getAssets().open("chatFace" + File.separator + name);
    ...
    public static void importCacheImgs(final Context context) {
        ...
        try {
            ...
            for (int i = 0; i < names.length; i++) {
                name = names[i];
                open = context.getAssets().open("img" + File.separator + name);
    ...
    public static List<SubUserInfo> importSubs(Context context) {
        BufferedReader reader = null;
        String line = null;
        StringBuffer sbf = new StringBuffer();
        String json = null;
        TestSubList testSubList = null;
        try {
            InputStream open = context.getAssets().open("json" + File.separator + "allSubUserInfo.json");
            ...
            json = sbf.toString();
            Gson gson = new Gson();
            testSubList = gson.fromJson(json, TestSubList.class);
            return testSubList.getData();
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            if (reader != null) {
