...
public class JazzyViewPager extends ViewPager {
    ...
    /*默认角度*/
    private float cubeDegrees = 90F;

    public float getCubeDegrees() {
        return cubeDegrees;
    }

    public void setCubeDegrees(float cubeDegrees) {
        this.cubeDegrees = cubeDegrees;
    }

    /*modify by 20151123 In order to fit more degree */
    private void animateCube(View left, View right, float positionOffset, boolean in) {
        if (mState != State.IDLE) {
            if (left != null) {
                manageLayer(left, true);
                mRot = (in ? cubeDegrees : - cubeDegrees) * positionOffset;
            ...
            if (right != null) {
                manageLayer(right, true);
                mRot = -(in ? cubeDegrees : -cubeDegrees) * (1 - positionOffset);
